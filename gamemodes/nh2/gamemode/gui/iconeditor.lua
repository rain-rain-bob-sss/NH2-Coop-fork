...
function PANEL:Init()
    ...
    bbox.LoadCookies = function(p)
        local b = p:GetCookie("checkbox_checked")
        p:SetChecked(b)
        p:OnChange(tobool(b))
    end
...
function PANEL:Refresh()
    ...
    self.ModelPanel.LayoutEntity = function()
        self:UpdateEntity(self.ModelPanel:GetEntity())
    end
...
function PANEL:FillAnimations(ent)
    ...
    if ent:SkinCount() > 1 then
        ...
        skinSlider.OnValueChanged = function(s, newVal)
            ...
            if IsValid(self:GetOrigin()) then
                self:GetOrigin():SkinChanged(newVal)
            end
    ...
    for k = 0, ent:GetNumBodyGroups() - 1 do
        ...
        bgSlider.OnValueChanged = function(s, newVal)
            ...
            if IsValid(self:GetOrigin()) then
                self:GetOrigin():BodyGroupChanged(s.BodyGroupID, newVal)
            end
