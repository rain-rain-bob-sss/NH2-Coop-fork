...
local function GetAllFiles(tab, folder, extension, path)
    ...
    if totalCalls >= expectedCalls then
        notification.Kill("SandboxSearchIndexing")
    end
end
...
hook.Add("Think", "sandbox_queued_search", function()
    ...
    if not timer.Exists("search_models_update") or #queuedSearch < 1 then
        timer.Create("search_models_update", 1, 1, function()
...
search.AddProvider(function(str)
    ...
    for k, v in pairs(model_list) do
        ...
        if modelpath:StartWith("models/") and modelpath:EndsWith(".mdl") and not str:EndsWith(".mdl") then
            modelpath = modelpath:sub(8, modelpath:len() - 4)
        end
...
local function AddSearchProvider(listname, ctype, stype)
    search.AddProvider(function(str)
        ...
        for k, v in pairs(entities) do
            ...
            if not name and not name_c then continue end

            if (name and name:lower():find(str, nil, true)) or (name_c and name_c:lower():find(str, nil, true)) then
                local entry = {
