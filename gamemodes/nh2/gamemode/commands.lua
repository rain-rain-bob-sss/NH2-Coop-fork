...
local function InternalSpawnNPC(ply, Position, Normal, Class, Equipment, SpawnFlagsSaved, NoDropToFloor)
    ...
    if NPCData.OnCeiling then
        if Vector(0, 0, -1):Dot(Normal) < 0.95 then return nil end
        --
...
local function GenericNPCDuplicator(ply, mdl, class, equipment, spawnflags, data)
    ...
    if NPCData and NPCData.OnCeiling then
        normal = Vector(0, 0, -1)
    end
...
-- Huuuuuuuuhhhh
local function AddNPCToDuplicator(class)
    duplicator.RegisterEntityClass(class, GenericNPCDuplicator, "Model", "Class", "Equipment", "SpawnFlags", "Data")
end
